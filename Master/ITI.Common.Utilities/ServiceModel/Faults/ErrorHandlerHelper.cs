...
namespace ITI.Common.Utilities.ServiceModel.Faults
{
    public static class ErrorHandlerHelper
    {
        ...
        {
            Entry entry = CreateLogEntry(error, fault);
            try
            {
        ...
        static Entry CreateLogEntry(Exception error, MessageFault fault)
        {
            ...
            return new Entry(assemblyName, fileName, lineNumber, typeName, methodName, exceptionName, exceptionMessage, providedFault, providedMessage);
        }
        static Entry CreateLogEntry(Exception error)
        {
            return CreateLogEntry(error, null);
        }
