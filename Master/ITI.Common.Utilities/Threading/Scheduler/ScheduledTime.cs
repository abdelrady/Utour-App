...
namespace ITI.Common.Utilities.Threading.Scheduler
{
	...
	public class ScheduledTime : IScheduledItem
    {
		...
		public ScheduledTime(string StrBase, string StrOffset)
		{
			...
			m_Base = (EventTimeBase)Enum.Parse(typeof(EventTimeBase), StrBase, true);
			Init(StrOffset);
        }
        #endregion
        ...
        private void Init(string StrOffset)
        {
            ...
            {
                case EventTimeBase.BySecond:
                    m_Offset = new TimeSpan(0, 0, 0, 0, int.Parse(StrOffset));
                    break;
                case EventTimeBase.ByMinute:
                    string[] ArrMinute = StrOffset.Split(',');
                    m_Offset = new TimeSpan(0, 0, 0, ArrayAccess(ArrMinute, 0), ArrayAccess(ArrMinute, 1));
                    break;
                case EventTimeBase.Hourly:
                    string[] ArrHour = StrOffset.Split(',');
                    m_Offset = new TimeSpan(0, 0, ArrayAccess(ArrHour, 0), ArrayAccess(ArrHour, 1), ArrayAccess(ArrHour, 2));
                    break;
                case EventTimeBase.Daily:
                    DateTime Daytime = DateTime.Parse(StrOffset);
                    m_Offset = new TimeSpan(0, Daytime.Hour, Daytime.Minute, Daytime.Second, Daytime.Millisecond);
                    break;
                case EventTimeBase.Weekly:
                    ...
                    m_Offset = new TimeSpan(int.Parse(ArrWeek[0]), WeekTime.Hour, WeekTime.Minute, WeekTime.Second, WeekTime.Millisecond);
                    break;
                case EventTimeBase.Monthly:
                    ...
                    m_Offset = new TimeSpan(int.Parse(ArrMonth[0]) - 1, MonthTime.Hour, MonthTime.Minute, MonthTime.Second, MonthTime.Millisecond);
                    break;
                default:
                    throw new Exception("Invalid base specified for timer.");
            }
		...
		{
            DateTime NextRun = LastSyncForTime(time) + m_Offset;
